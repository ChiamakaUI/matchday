...
import { agentVaultPda, agentConfigPda, getConnection } from '@/lib/solana';
import { agentApi } from '@/lib';
import { PublicKey } from '@solana/web3.js';
import {
  Bot, Zap, Shield, Target, ChevronRight, Check, Loader2,
} from 'lucide-react';
...
export default function AgentPage() {
  ...
  const handleDeposit = async (amount: number) => {
    ...
    const token = await getToken();
    await agentApi.updateSettings({ totalDeposited: amount, depositTx: tx }, token!);
    await queryClient.invalidateQueries({ queryKey: ['agent'] });
  };

  const handleAddRule = async (ruleType: AgentRuleType, ruleValue: Record<string, unknown>) => {
    const token = await getToken();
  ...
  return (
    <AuthLayout>
      <div className="space-y-6 max-w-2xl">
        <div>
          <h1 className="text-xl font-display font-bold text-text-primary">AI Agent</h1>
          <p className="text-sm text-zinc-500 mt-1">
            Your agent watches open contests, builds predictions, and enters on your behalf.
          </p>
        </div>
...
const DEFAULT_RULES = [
  {
    ...
    description: 'Agent uses a balanced approach — not too safe, not too risky.',
  ...
  },
  {
    ...
    description: 'Balanced prediction selection between safe and aggressive picks.',
...
function AgentSetupWizard() {
  ...
  const handleSetup = async () => {
    ...
    try {
      ...
      const existingConfig = await connection.getAccountInfo(agentConfigPda(userPubkey));
      ...
      const budget = await agentApi.setup(
        { maxSpendPerContest: maxSpend, maxContestsPerWeek: maxContests, vaultPda },
        token!,
      ) as any;
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      const token = await getToken();
      await agentApi.updateSettings({ totalDeposited: depositAmount, depositTx: tx }, token!);
      setStep(3);
    } catch (e: unknown) {
  ...
  const handleSaveRules = async () => {
    ...
    try {
      if (budgetId) {
        const token = await getToken();
        const rulesToAdd = DEFAULT_RULES.filter((r) => selectedRules.has(r.key));
        for (const r of rulesToAdd) {
          await agentApi.addRule(
            { budgetId, ruleType: r.key, ruleValue: r.value as Record<string, unknown> },
            token!,
          );
        }
      }
      ...
      setErr(e instanceof Error ? e.message : 'Saving rules failed. Try again.');
    } finally {
      setLoading(false);
    }
  };
        ...
        >
          <div className="space-y-5">
            <SliderField
              ...
              value={maxSpend} min={1} max={50} step={1}
            ...
            <SliderField
              ...
              value={maxContests} min={1} max={10} step={1}
              format={(v) => `${v} contest${v > 1 ? 's' : ''}`}
              onChange={setMaxContests}
              hint="The agent won't enter more than this many contests per week."
            />
            <SummaryBox>
              Maximum weekly exposure: <span className="text-text-primary font-display font-bold">{formatUsdc(maxSpend * maxContests)}</span>
            </SummaryBox>
        ...
        >
          <div className="space-y-5">
            <SliderField
              ...
              value={depositAmount} min={5} max={500} step={5}
              format={(v) => formatUsdc(v)}
            ...
            <ActionButton onClick={handleDeposit} loading={loading}>
              Deposit {formatUsdc(depositAmount)} <ChevronRight className="h-4 w-4" />
            </ActionButton>
            <button
              onClick={() => setStep(3)}
                ...
                return (
                  ...
                  >
                    <div className={cn(
                      ...
                      on ? 'bg-gold-400 border-gold-400' : 'border-white/20',
                    )}>
                      {on && <Check className="h-3 w-3 text-text-primary" />}
                    </div>
                    <div className="min-w-0">
                      <p className={cn('text-sm font-medium', on ? 'text-text-primary' : 'text-zinc-400')}>
                        {r.label}
                      </p>
        ...
        >
          <div className="space-y-5">
            <div className="rounded-xl bg-white/5 divide-y divide-white/5">
              <SummaryRow label="Max per contest" value={formatUsdc(maxSpend)} />
              <SummaryRow label="Max per week" value={`${maxContests} contest${maxContests > 1 ? 's' : ''}`} />
              <SummaryRow label="Weekly budget" value={formatUsdc(maxSpend * maxContests)} />
              <SummaryRow label="Vault funded" value={formatUsdc(depositAmount)} accent />
              <SummaryRow label="Rules" value={`${selectedRules.size} rule${selectedRules.size !== 1 ? 's' : ''} configured`} />
            </div>
            {err && <p className="text-xs text-wrong">{err}</p>}
          ...
          <div key={step.id} className="flex items-center flex-1 last:flex-none">
            <div className="flex flex-col items-center gap-1.5">
              <div className={cn(
                ...
                done ? 'bg-gold-400 text-text-primary'
                  : active ? 'bg-gold-400/20 border border-gold-400/50 text-gold-400'
              ...
              <span className={cn(
                ...
                active ? 'text-gold-400' : done ? 'text-zinc-400' : 'text-zinc-700',
              )}>
                {step.label}
              </span>
            </div>
            {i < STEPS.length - 1 && (
              <div className={cn('flex-1 h-px mx-2 mb-4 transition-colors', done ? 'bg-gold-400/50' : 'bg-white/8')} />
            )}
          </div>
        );
      })}
...
function SliderField({ label, value, min, max, step, format, onChange, hint }: {
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-baseline">
        ...
        <span className="text-xl font-black text-gold-400">{format(value)}</span>
      </div>
      <input
...
function SummaryRow({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="flex justify-between items-center px-4 py-3">
      ...
      <span className={cn('text-sm font-display font-bold', accent ? 'text-gold-400' : 'text-text-primary')}>{value}</span>
    </div>
  );
}

function ActionButton({ onClick, loading = false, children }: {
