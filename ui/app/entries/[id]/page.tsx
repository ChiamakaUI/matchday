...
//   // Live score updates — refetch entry when scores change
//   const handleScoreUpdate = useCallback(
//     (event: LiveScoreEvent) => {
//       if (!entry) return;
//       // Check if this fixture is in our entry's predictions
//       const relevant = entry.predictions.some(
//         (p) => p.fixtureId === event.fixtureId,
//       );
//       if (relevant) {
//         // Refetch entry data to get updated scores and prediction results
...
//   // Count results
//   const correctCount = entry.predictions.filter(
//     (p) => p.isCorrect === true,
//   ).length;
//   const wrongCount = entry.predictions.filter(
//     (p) => p.isCorrect === false,
//   ).length;
//   const pendingCount = entry.predictions.filter(
//     (p) => p.isCorrect === null,
//   ).length;
...
//           My Entries
//         </Link>
//         <h1 className="text-2xl font-display font-bold">{entry.contestName}</h1>

//         {/* Score summary */}
//         <div className="flex items-center gap-3 mt-3">
//           <div className="bg-gold-400/20 text-gold-400 px-4 py-1.5 rounded-full text-sm font-display font-bold">
...
//       {/* Predictions grouped by fixture */}
//       <div className="space-y-4">
//         {Array.from(fixtureMap.entries()).map(([fixtureId, predictions]) => {
//           const first = predictions[0]!;
//           const finished = first.fixtureStatus
//             ? isFixtureFinished(first.fixtureStatus)
//             : false;
//           const live = first.fixtureStatus
//             ? isFixtureLive(first.fixtureStatus)
//             : false;
//           const fixturePoints = predictions.reduce(
...
//                 live ? "border-green-800" : "border-border",
//               )}
//             >
//               {/* Fixture header */}
//               <div
...
//                   live ? "border-green-800 bg-green-950/30" : "border-border",
//                 )}
//               >
//                 <div className="flex items-center gap-3">
//                   <span className="font-display font-display font-bold">{first.homeTeamName}</span>
//                   {(finished || live) && first.homeScore !== null ? (
//                     <span className="text-lg font-display font-bold text-text-primary min-w-[50px] text-center">
//                       {first.homeScore} – {first.awayScore}
//                     </span>
//                   ) : (
//                     <span className="text-xs text-text-tertiary min-w-[50px] text-center">
//                       vs
//                     </span>
//                   )}
//                   <span className="font-display font-display font-bold">{first.awayTeamName}</span>
//                 </div>
...
//         <h3 className="font-display font-display font-bold mb-3">Scoring Reference</h3>
...
//         resolved && correct && "bg-green-950/20",
//         resolved && !correct && "bg-red-950/10",
//       )}
//     >
//       <div className="flex items-center gap-3">
...
//             {predictionTypeLabel(prediction.predictionType)}
//           </p>
...
//               <span
//                 className={cn(
//                   "ml-2 text-xs font-display font-bold",
...
//               correct ? "text-gold-400" : "text-wrong",
//             )}
//           >
//             {correct ? `+${prediction.pointsAwarded}` : "0"} pts
//           </span>
//         ) : (
  ...
  return (
    <AuthLayout>
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-display font-display font-bold tracking-tight">{entry.contestName}</h1>

        <div className="flex items-center gap-3 mt-3">
          <div className="bg-gold-400/15 text-gold-400 px-4 py-1.5 rounded-full text-sm font-mono font-display font-bold">
            {formatPoints(entry.totalPoints)}
          </div>
          {entry.rank && (
            <span className="text-text-secondary text-sm">Rank #{entry.rank}</span>
          ...
          return (
            ...
            >
              <div className={cn(
                ...
                <div className="flex items-center gap-3">
                  <span className="font-display font-display font-display font-bold">{first.homeTeamName}</span>
                  {(finished || live) && first.homeScore !== null ? (
                  ...
                  <span className="font-display font-display font-display font-bold">{first.awayTeamName}</span>
                </div>
                <div className="flex items-center gap-2">
                  {(finished || live) && fixturePoints > 0 && (
                    <span className="font-mono font-display font-bold text-sm text-correct">
                      +{fixturePoints}
                    </span>
                  )}
                  {live && (
      ...
      <div className="mt-8 bg-surface-card border border-border rounded-xl p-5">
        <h3 className="font-display font-display font-display font-bold mb-3">Scoring Reference</h3>
        <div className="grid grid-cols-2 gap-2 text-sm">
          <div className="text-text-secondary">Match Result</div>
...
function PredictionRow({ prediction }: { prediction: Prediction }) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-text-secondary">
            ...
            {prediction.confidence > 1 && (
              <span className={cn(
                'ml-1.5 text-xs font-mono font-display font-bold',
      ...
      <div className="text-right">
        {resolved ? (
          <span className={cn('font-mono font-display font-bold text-sm', correct ? 'text-correct' : 'text-wrong')}>
