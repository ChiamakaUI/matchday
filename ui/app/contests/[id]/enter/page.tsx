...
import { AuthLayout, PredictionInputGroup } from '@/components';
import { useContest, useUserEntries, useAuth, usePredictionBuilder, useSubmitEntry } from '@/hooks';
import { formatUsdc, formatKickoff, formatPoints, isDeadlinePassed, signEnterContestTx } from '@/lib';
    ...
    <AuthLayout>
      <div className="pb-32">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-display font-bold tracking-tight">{contest.name}</h1>
          <p className="text-text-secondary mt-1">
            {fixtures.length} fixture{fixtures.length !== 1 ? 's' : ''} • Entry fee{' '}
            <span className="text-gold-400 font-display font-bold">{formatUsdc(contest.entryFee)}</span>
          </p>
        </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="text-right min-w-[100px]">
                      <p className="font-display font-bold">{fixture.homeTeamName}</p>
                    </div>
                    <div className="text-center">
                      <span className="text-xs text-text-muted uppercase font-mono">vs</span>
                    </div>
                    <div className="min-w-[100px]">
                      <p className="font-display font-bold">{fixture.awayTeamName}</p>
                    </div>
                  </div>
        ...
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-6">
            <div>
              ...
              <p className="font-display font-bold">{builder.totalPredictions}</p>
            </div>
            <div>
              <p className="text-[10px] text-text-muted uppercase font-mono tracking-wider">Potential</p>
              <p className="font-display font-bold text-gold-400">{formatPoints(builder.potentialPoints)}</p>
            </div>
            <div>
              <p className="text-[10px] text-text-muted uppercase font-mono tracking-wider">Entry Fee</p>
              <p className="font-display font-bold">{formatUsdc(contest.entryFee)}</p>
            </div>
          </div>
